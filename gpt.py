...
apprv = [1749293946,5854453255,1572967955]
global limi
limi = 15
    ...
    async def handler(event):
        if event.sender_id in [5940840757,5955887728,5778624772,6276768397]:
            if event.reply_to:
                ...
                yyyy = yyy.id
                if yyyy == 1749293946:
                    DEST = open("dest.txt","r+")
                    trgt = int(DEST.read().replace(" ","").replace("\n",""))
                    DEST.close
                    if event.media:
...
ریست کردن محدودیت های اعضا(مخصوص ادمین)''', parse_mode="HTML",reply_to=message.id)
        if message.text.startswith("_dest"):
            DEST = open("dest.txt","w+")
            DEST.write(str(message.text.split(" ")[1]))
            DEST.close()
